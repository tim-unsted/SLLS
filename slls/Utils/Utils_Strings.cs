...
namespace slls.Utils
{
  ...
  internal partial class wwUtils
  {
    ...
    public static string DisplayMemoEncoded(string Text)
    {
      ...
      Text = slls.Utils.wwUtils.DisplayMemo(HttpUtility.HtmlEncode(Text));
    ...
    {
      return "<a href=\"" + Url + "\" " + (string.IsNullOrEmpty(Target) ? string.Empty : "target=\"" + Target + "\" ") + (string.IsNullOrEmpty(AdditionalMarkup) ? string.Empty : AdditionalMarkup) + ">" + Text + "</a>";
    }
  ...
  public class ExpandUrlsParser
  {
    ...
    {
      string pattern = @"([string.Empty'=]|&quot;)?(http://|ftp://|https://|www\.|ftp\.[\w]+)([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])";

      // string pattern = @"[string.Empty'=]?(http://|ftp://|https:/[\w]+)([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])|((www\.)|(ftp\.)).*?((\.com)|(\.net)|(\.org))((.|\?|\#)+\s)?";        
      // string pattern = @"[string.Empty'=]?(http://|ftp://|https://|mailto:)[\w]+(.[\w]+)([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])|((www\.)|(ftp\.)).*?((\.com)|(\.net)|(\.org))";
