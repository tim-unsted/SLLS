        ...
        $.ajax({
            ...
            success: function(data) {
                for (var i = 0; i < data.length; i++) {
                    title[i] = { copyId: data[i].CopyID, title: data[i].Title, copyNumber: data[i].CopyNumber, edition: data[i].Edition, year: data[i].Year, authors: data[i].Authors };
                }
            }
        });
        response(title);
